...
const StoreItem = (props) => {
  ...
  const [redemptionIsSuccess, setRedemptionIsSuccess] = React.useState(false);
  const { handleItemRedemption } = useFirebase();
  const user = useSelector((state) => state.user);
  const openModal = () => {
    if (props.itemPoints > user.points) {
      console.log("ssss");
      return;
    }
  ...
  const itemClickHandler = () => {
    // props.updateUser(props.itemPoints, props.name);
    handleItemRedemption(props.name, props.itemPoints);
    setRedemptionIsSuccess(true);
  };

  const closeModalHandler = () => {
    setModalIsActive(false);
    setRedemptionIsSuccess(false);
  };

  const eligableItemSuccessContent = (
    <React.Fragment>
      ...
      <div className="promo-code">
        <p className="warning">
          Please show this to the cashier before closing.
        </p>
        <div className="content">
          {" "}
          You've earned one free{" "}
          <span className="code-number">{props.name}</span>
        </div>
      </div>
      <div className="buttons-container redemption-success">
        <h3>
          {" "}
          Your new total is now{" "}
          <span className="point-value">{user.points}</span> points.
        </h3>
        <button onClick={closeModalHandler} className="blue small">
          Continue
        </button>
      </div>
    </React.Fragment>
  ...
  const eligableItemContent = redemptionIsSuccess ? (
    eligableItemSuccessContent
  ) : (
    <React.Fragment>
      <h2>
        <span className="point-value">{props.name}</span> Redemption
      </h2>
      <div className="redemption-details" />
      <span className="confirm-text">
        Redeem for <span className="point-value">{props.itemPoints}</span>{" "}
        points?
      </span>
      <div className="buttons-container">
        <button onClick={closeModalHandler} className="small cancel-btn">
          Actually, Nevermind
        </button>
        <button onClick={() => itemClickHandler()} className="blue small">
          Yes I'm Sure!
        </button>
      </div>
    </React.Fragment>
  );
  const nonEligableItemContent = <React.Fragment>Non Eligable</React.Fragment>;

  const modalContent = eligableItemContent;
  return (
    <React.Fragment>
      <Modal
        className="store-item-modal"
        show={modalIsActive}
      ...
      <div
        className={`store-item-wrapper ${
          props.itemPoints <= user.points ? "eligable" : "not-eligable"
        }`}
      >
        ...
        <div className="store-item__points">
          <p id="points">{props.itemPoints} Points</p>
        </div>

        <button onClick={() => openModal()} className="blue small">
          {props.itemPoints <= user.points
            ? "REDEEM"
            : `${props.itemPoints - user.points} Points Away`}
        </button>
      </div>
    </React.Fragment>
  );
