...
const StoreItem = (props) => {
	...
	const openModal = () => {
		if (props.itemPoints > props.userPoints) {
			console.log('ssss');
			return;
		}
	...
	const eligableItemSuccessContent = (
		<React.Fragment>
			...
			<div className="promo-code">
				<p className="warning">Please show this to the cashier before closing.</p>
				<div className="content">
					{' '}
					You've earned one free <span className="code-number">{props.name}</span>
				</div>
			</div>
			<div className="buttons-container redemption-success">
				<h3>
					{' '}
					Your new total is now <span className="point-value">{props.userPoints}</span> points.
				</h3>
	...
	const eligableItemContent =
		redemptionIsSuccess ? eligableItemSuccessContent :
		<React.Fragment>
			<h2>
			...
			<span className="confirm-text">
				Redeem for <span className="point-value">{props.itemPoints}</span> points?
			</span>
			<div className="buttons-container">
				<button onClick={closeModalHandler} className="small">
					Actually, Nevermind
				</button>
	...
	return (
		<React.Fragment>
			<Modal show={modalIsActive} onCancel={closeModalHandler}>
				{modalContent}
			</Modal>
			<div
				className={`store-item-wrapper ${
					props.itemPoints <= props.userPoints ? 'eligable' :
					'not-eligable'}`}
			>
				...
				<button onClick={() => openModal()} className="blue small">
					{
						props.itemPoints <= props.userPoints ? 'REDEEM' :
						`${props.itemPoints - props.userPoints} Points Away`}
				</button>
			</div>
		</React.Fragment>
	);
