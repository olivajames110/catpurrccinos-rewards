...
const CheckInModal = (props) => {
  ...
  const titleText = "Points Earned!";
  const titleDescText =
    "Thanks for stopping by! Redeem your points at any time. Please wait atleast 24 hours until your next check-in.";

  const bodyText = "Catpurrccino Points";

  const pointNumber = (
    <h3>
      +
      <AnimatedNumber
        value={props.checkinPointValue}
        duration={400}
  ...
  const checkInContent = (
    <div className="check-in-success">
      <div className="success__header">
        ...
        <h1>{props.isLoading ? "Please wait..." : titleText}</h1>

        <p>{!props.isLoading && titleDescText}</p>
      </div>
      <div className="success__body">
        <div className="new-points">
          {!props.isLoading && pointNumber}
          <span>
            {props.isLoading ? "Attempting to check you in." : bodyText}
          </span>
        </div>
      </div>
    </div>
